...
const Acount = () => {
    ...
    useEffect(() =>{
        ...
        fetch(`https://stawro.xyz/valid/get/01/data/paid/module/datad/${username}?key=${process.env.REACT_APP_API_KEY}`)
        .then(res => res.json())
        .then(user => setCoup(user))

        fetch(`https://stawro.xyz/valid/get/01/data/paid/module/datad/1/${username}?key=${process.env.REACT_APP_API_KEY}`)
        .then(res => res.json())
